...
class sample_dataset(Dataset):
    ...
    def __init__(self, seed: int = 12345,
                 n_series: int = 128, datapoints: int = 100):
        """
    ...
    def _generate_dataset(self):
        ...
        for i in range(self.n_series):
            ...
            check_size = 0
            while check_size < self.datapoints:
                ...
                check_size = len(table.index)

            
            sequence = table['Log Return'].values
            size = table['Log Return'].values.size

            random_pos = np.random.randint(0,size-self.datapoints)
            dataset.append([sequence[random_pos:random_pos+self.datapoints]])

        dataset = np.array(dataset)
        dataset = np.reshape(dataset,(self.n_series,self.datapoints,1))
        #3D/2D input
        # dataset = dataset[:, :, 0] 
